...
static void led_heartbeat_function(struct timer_list *t)
{
	...
	printk("[Fanning] led_heartbeat_function\n");
...
static int heartbeat_trig_activate(struct led_classdev *led_cdev)
{
	...
	printk("[Fanning] heartbeat_trig_activate\n");
...
static struct led_trigger v3s_led_trigger = {
	.name     = "v3snbv2",
...
{
	led_trigger_unregister(&v3s_led_trigger);
	return NOTIFY_DONE;
}
...
static int __init v3s_trig_init(void)
{
	printk("[Fanning] v3s_trig_init\n");

	int rc = led_trigger_register(&v3s_led_trigger);
...
static void __exit v3s_trig_exit(void)
{
...
module_init(v3s_trig_init);
module_exit(v3s_trig_exit);
